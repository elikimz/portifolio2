...
const Projects: React.FC = () => {
  ...
  const [updateProject, { error: updateError, isLoading: isUpdating }] =
    useUpdateProjectMutation();
  const [deleteProject, { error: deleteError, isLoading: isDeleting }] =
    useDeleteProjectMutation();
  ...
  const renderProject = useMemo(
    () => (project: Project) =>
      (
        <li
          key={project.id}
          className="mb-4 p-4 bg-gray-800 rounded border border-gray-700 relative"
        >
          {editingProject?.id === project.id ? (
            <div>
              <input
            ...
            <div>
              ...
              {project.image_url ? (
                ...
                <p>
                  <strong>Image URL:</strong> No Image Available
                </p>
              )}
              <button
                onClick={() => handleEditProject(project)}
    ...
    [editingProject, isUpdating, isDeleting]
  );
